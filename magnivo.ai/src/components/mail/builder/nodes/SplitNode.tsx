'use client'

import { Handle, Position, type NodeProps } from '@xyflow/react'
import { Split } from 'lucide-react'
import type { CampaignBuilderNode } from '@/types/campaign'
import { useCampaignBuilderStore } from '@/stores/campaign-builder'
import { getNodeSeverity } from '@/lib/campaign-builder/validation'

export default function SplitNode({ data, selected, id }: NodeProps<CampaignBuilderNode>) {
  const nodes = useCampaignBuilderStore((s) => s.nodes)
  const edges = useCampaignBuilderStore((s) => s.edges)
  const severity = getNodeSeverity(id, nodes, edges)

  const borderClass = severity === 'error'
    ? 'border-red-500/60 ring-2 ring-red-500/20'
    : selected
      ? 'border-blue-500/60 ring-2 ring-blue-500/20'
      : 'border-cyan-500/30'

  const pcts = (data.percentages as number[]) || [50, 50]

  return (
    <div className={`min-w-[220px] rounded-xl border p-3 bg-white dark:bg-card shadow-sm transition-all ${borderClass}`}>
      <Handle type="target" position={Position.Top} className="!w-2.5 !h-2.5 !bg-gray-400 !border-2 !border-white dark:!border-card" />
      <div className="flex items-center gap-2.5">
        <div className="h-8 w-8 rounded-lg bg-cyan-500/10 text-cyan-600 grid place-items-center">
          <Split className="h-4 w-4" />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wide font-medium">Split</p>
          <p className="text-sm font-semibold truncate">{data.label}</p>
        </div>
      </div>
      <div className="mt-2 flex gap-1">
        {pcts.map((p, i) => (
          <div key={i} className="flex-1 text-center text-[10px] text-muted-foreground">
            <div className="h-1.5 rounded-full bg-cyan-400/40 mb-0.5" style={{ width: `${p}%` }} />
            {p}%
          </div>
        ))}
      </div>
      <Handle
        type="source"
        position={Position.Bottom}
        id="variant-a"
        className="!w-2.5 !h-2.5 !bg-cyan-400 !border-2 !border-white dark:!border-card !left-[30%]"
      />
      <Handle
        type="source"
        position={Position.Bottom}
        id="variant-b"
        className="!w-2.5 !h-2.5 !bg-cyan-400 !border-2 !border-white dark:!border-card !left-[70%]"
      />
    </div>
  )
}
