...
import { Radio, Link2, Unlink, RefreshCw, Loader2, XCircle } from 'lucide-react'
import { cn } from '@/lib/utils'
import { getSndsDomains, connectSndsDomainAction, disconnectSndsDomainAction, syncSndsMetricsAction, getSndsDashboard } from '@/app/actions/deliverability'
import type { SndsDomain, SndsDashboardStats } from '@/types/deliverability'
...
export function SndsDashboard() {
  const [domains, setDomains] = useState<SndsDomain[]>([])
  const [stats, setStats] = useState<SndsDashboardStats | null>(null)
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    const [d, s] = await Promise.all([getSndsDomains(), getSndsDashboard()])
    setDomains(d)
    setStats(s)
    setLoading(false)
  }, [])
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    await connectSndsDomainAction(connectDomain.trim())
    setConnectDomain('')
  ...
  const handleSync = async (id: string) => {
    setSyncing(id)
    await syncSndsMetricsAction(id)
    setSyncing(null)
    await loadData()
  }
  ...
  const handleDisconnect = async () => {
    ...
    await disconnectSndsDomainAction(disconnectConfirmId)
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-sm font-medium flex items-center gap-2">
          <Radio className="h-4 w-4" /> Microsoft SNDS
        </CardTitle>
      ...
      <CardContent>
        {stats && (
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">{stats.totalTrapHits}</div>
              <div className="text-xs text-muted-foreground">Trap Hits</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-amber-600">{stats.avgComplaintRate > 0 ? `${(stats.avgComplaintRate * 100).toFixed(2)}%` : 'N/A'}</div>
        ...
        {domains.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-4">No SNDS domains connected</p>
        ) : (
                ...
                <div key={domain.id} className="flex items-center gap-3 p-2 rounded-lg border">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium">{domain.sndsDomain}</span>
            ...
            <Card className="w-full max-w-md mx-4">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base">Connect Microsoft SNDS</CardTitle>
              ...
              <CardContent>
                <form onSubmit={handleConnect} className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    You will be redirected to Microsoft to authorize SNDS access for IP reputation data.
                  </p>
            ...
            <Card className="w-full max-w-sm mx-4">
              ...
              <CardContent className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  Disconnect this domain from Microsoft SNDS? Historical data will be preserved but sync will stop.
                </p>
                <div className="flex gap-2 justify-end">
