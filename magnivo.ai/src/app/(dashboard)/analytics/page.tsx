...
import { getLlmUsageSummary, getMarketSizing, getIcps, getRevenueForecast, getBoardReport } from '@/app/actions/gtm'
...
export default async function AnalyticsPage(props: { searchParams: Promise<{ days?: string }> }) {
    ...
    const [data, usage, market, icps, forecast, boardReport] = await Promise.all([
        getAnalytics(days),
        getLlmUsageSummary(),
        getMarketSizing(),
        getIcps(),
    ...
    return (
        <>
            ...
            <div className="max-w-7xl mx-auto px-0 pb-12">
                <GtmAnalytics usage={usage} market={market} icps={icps} forecast={forecast} boardReport={boardReport} />
            </div>
        </>
    )
}
