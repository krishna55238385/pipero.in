"""Same BYO-LLM routing tests as phase1's — phase2's connector mirrors
phase1's structure exactly, so covered separately here to catch any
...
def openai_module_factory(monkeypatch):
    def _load():
        import gtm_backend.phase2.connectors.openai as mod
...
def test_chat_completion_routes_through_openrouter_when_configured(openai_module_factory, monkeypatch, mocker):
    ...
    assert create_mock.call_args.kwargs["model"] == "anthropic/claude-haiku-4.5"
