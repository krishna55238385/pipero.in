...
#
#   bash gtm_backend/gtm_service/run-local.sh
#
# Then make sure magnivo.ai/.env.local has:
...
exec "$REPO_ROOT/.venv/bin/python" -m uvicorn gtm_backend.gtm_service.app:app --host 0.0.0.0 --port "${PORT:-8080}"
