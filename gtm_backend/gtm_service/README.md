...
A thin FastAPI service that lets the **Magnivo AI CRM** run the Python
...
CRM button → Next.js server action (actions/gtm.ts)
           ...
           → subprocess `python -m gtm_backend.phase1 run-all ...` with GTM_ORG_ID set
...
2. Render → New → Blueprint → pick `gtm_backend/gtm_service/render.yaml`.
...
Railway: create a service from `gtm_backend/gtm_service/Dockerfile` (build context = repo root)
