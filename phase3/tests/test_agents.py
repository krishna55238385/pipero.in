...
from phase3.agents.agent_14_orchestrator import run_orchestration
...
def test_agent_11_writes_personalisation(sample_lead, mock_llm_personalisation):
    ...
    with patch(
        "phase3.agents.agent_11_personalisation.supabase.get_leads_for_personalisation",
        return_value=[sample_lead],
    ), patch(
        "phase3.agents.agent_11_personalisation.llm.chat_json",
        return_value=mock_llm_personalisation,
    ) as llm_mock, patch(
        "phase3.agents.agent_11_personalisation.supabase.upsert_personalisation",
        return_value=None,
    ) as upsert_mock:
...
def test_agent_12_writes_sequence(sample_personalisation_row, mock_llm_sequence):
    ...
    with patch(
        "phase3.agents.agent_12_copywriter.supabase.get_personalisations",
        return_value=[sample_personalisation_row],
    ), patch(
        "phase3.agents.agent_12_copywriter.llm.chat_json",
        return_value=mock_llm_sequence,
    ) as llm_mock, patch(
        "phase3.agents.agent_12_copywriter.supabase.upsert_sequence",
        return_value=None,
    ) as upsert_mock:
...
def test_agent_13_writes_channel_plan_and_clamps_window(sample_sequence_row):
    ...
    with patch(
        "phase3.agents.agent_13_channel_strategy.supabase.get_sequences",
        return_value=[sample_sequence_row],
    ), patch(
        "phase3.agents.agent_13_channel_strategy.supabase.get_icp",
        return_value={"id": 1, "name": "Test ICP", "industry": ["HR Tech"], "geography": ["India"]},
    ), patch(
        "phase3.agents.agent_13_channel_strategy.supabase.get_account_intel_for_lead",
        return_value={"company_size_estimate": "Mid-market", "what_they_do": "HR"},
    ), patch(
        "phase3.agents.agent_13_channel_strategy.llm.chat_json",
        return_value=bad_llm,
    ), patch(
        "phase3.agents.agent_13_channel_strategy.supabase.upsert_channel_plan",
        return_value=None,
    ) as upsert_mock:
...
def test_agent_14_dry_run_writes_log_entries(
    ...
    with patch(
        "phase3.agents.agent_14_orchestrator.supabase.get_sequences",
        return_value=[sample_sequence_row],
    ), patch(
        "phase3.agents.agent_14_orchestrator.supabase.get_channel_plans",
        return_value=[sample_channel_plan_row],
    ), patch(
        "phase3.agents.agent_14_orchestrator.supabase.get_personalisations",
        return_value=[],
    ), patch(
        "phase3.agents.agent_14_orchestrator.supabase.get_leads_for_personalisation",
        return_value=[sample_lead],
    ), patch(
        "phase3.agents.agent_14_orchestrator.supabase.get_sent_lead_ids",
        return_value=set(),
    ), patch(
        "phase3.agents.agent_14_orchestrator.supabase.get_replied_lead_ids",
        return_value=set(),
    ), patch(
        "phase3.agents.agent_14_orchestrator.instantly.is_configured",
        return_value=True,
    ), patch(
        "phase3.agents.agent_14_orchestrator.instantly.create_campaign",
        side_effect=_exploding_create_campaign,
    ) as create_mock, patch(
        "phase3.agents.agent_14_orchestrator.instantly.add_leads_to_campaign",
        side_effect=_exploding_create_campaign,
    ), patch(
        "phase3.agents.agent_14_orchestrator.instantly.activate_campaign",
        side_effect=_exploding_create_campaign,
    ), patch(
...
def test_agent_15_no_campaigns_returns_clean_summary():
    ...
    with patch(
        "phase3.agents.agent_15_ab_testing.supabase.get_outreach_log",
        return_value=[],
    ), patch(
        "phase3.agents.agent_15_ab_testing.instantly.is_configured",
        return_value=True,
    ), patch(
        "phase3.agents.agent_15_ab_testing.instantly.get_campaign_analytics",
    ) as analytics_mock, patch(
        "phase3.agents.agent_15_ab_testing.supabase.upsert_ab_test_results",
    ) as upsert_mock:
