...
def test_agent_06_falls_back_to_website_when_search_empty(mocker, fake_lead):
    ...
    mocker.patch("phase2.agents.agent_06_account_intel.serpapi.search", return_value=[])
    mocker.patch("phase2.agents.agent_06_account_intel.serpapi.search_news", return_value=[])
    site_mock = mocker.patch(
        "phase2.agents.agent_06_account_intel.website.fetch_company_pages",
...
def test_agent_06_llm_knowledge_fallback_when_no_web_at_all(mocker, fake_lead):
    ...
    """
    from phase2.core.prompts import ACCOUNT_INTELLIGENCE_FALLBACK_SYSTEM
...
def test_agent_08_writes_competitor_card_per_discovery(mocker, fake_icp):
    ...
    mocker.patch(
        "phase2.agents.agent_08_competitive.supabase.get_leads_for_account_intel",
        return_value=[],
    )
...
def test_agent_08_falls_back_to_llm_when_search_empty(mocker, fake_icp):
    ...
    """
    from phase2.core.prompts import COMPETITOR_DISCOVERY_SYSTEM
    ...
    mocker.patch("phase2.agents.agent_08_competitive.serpapi.search", return_value=[])
    mocker.patch("phase2.agents.agent_08_competitive.serpapi.search_news", return_value=[])
...
def test_agent_07_complete_coverage_with_budget_champion(
    ...
    llm_payload = {
        "stakeholders": [
            {"full_name": "Priya Iyer", "job_title": "CEO", "role_type": "economic_buyer",
             "seniority": "C-suite", "confidence": "high", "rank": 1},
    ...
    llm_payload = {
        "stakeholders": [
            ...
            {"full_name": "Dev Rao", "job_title": "HR Manager", "role_type": "champion",
             "seniority": "Manager", "confidence": "medium", "rank": 2},
        ],
...
def test_agent_08_flags_lead_using_competitor(mocker, fake_icp, fake_lead):
    ...
    mocker.patch(
        "phase2.agents.agent_08_competitive.llm.chat_json", side_effect=fake_chat_json,
    )
    mocker.patch(
        "phase2.agents.agent_08_competitive.supabase.upsert_competitor", return_value=1,
    )
    mocker.patch(
        "phase2.agents.agent_08_competitive.supabase.get_leads_for_account_intel",
        return_value=[fake_lead],
    ...
    mocker.patch(
        "phase2.agents.agent_08_competitive.supabase.get_account_brief",
        return_value=brief,
    )
    usage_mock = mocker.patch(
        "phase2.agents.agent_08_competitive.supabase.upsert_lead_competitor_usage",
        return_value=1,
    )
...
def test_agent_10_approve_insights_for_one_lead(mocker):
    ...
    approve_mock = mocker.patch(
        "phase2.agents.agent_10_gtm_insights.supabase.approve_gtm_insight",
        return_value=1,
    )

    from phase2.agents.agent_10_gtm_insights import approve_insights
...
def test_agent_10_approve_all_pending(mocker):
    ...
    mocker.patch(
        "phase2.agents.agent_10_gtm_insights.supabase.get_pending_gtm_insights",
        return_value=[
